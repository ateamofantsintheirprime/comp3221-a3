...
class MyTCPServer(socketserver.ThreadingTCPServer):
	...
	def new_block_proposal(self):
		with self.blockchain_lock:
			bp = self.blockchain.block_proposal()
			self.block_proposals.add(json.dumps(bp))
			print("[PROPOSAL] Created a block proposal: {}".format(bp))
	...
	def responding_loop(self):
		while True:
			...
			if data['type'] == 'transaction':
				print("[NET] Received a transaction from node {}: {}".format(self.client_address[0], data['payload']))
				self.transaction_response(data['payload'])
			elif data['type'] == 'values':
				print("[BLOCK] Received a block request from node {}: {}".format(self.client_address[0], data['payload']))
